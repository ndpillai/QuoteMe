		...
		submitQuoteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				if (searchUserComboBox.getSelectedIndex()==0) {
					...
					Notification newQuoteNotification = new Notification(getSpeaker(), mainPanel.clientPanel.getCurrentUser().getUserName(), "New Quote", new Date(), quoteTextArea.getText(), newQuote);
					getSpeaker().addNotification(newQuoteNotification);
					mainPanel.clientPanel.quoteMeClient.sendObject(newQuoteNotification);
